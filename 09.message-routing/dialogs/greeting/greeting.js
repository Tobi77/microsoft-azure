...
class Greeting extends ComponentDialog {
    ...
    async promptForCityStep(step) {
        ...
        if (!greetingState.city) {
            return await step.prompt(CITY_PROMPT, `Hallo ${ greetingState.name }, aus welcher Stadt kommst du?`);
        } else {
            return await step.next();
        }
    }
    ...
    async greetUser(step) {
        ...
        await step.context.sendActivity(`Hi ${ greetingState.name }, aus ${ greetingState.city }, schön dich zu sehen!`);
