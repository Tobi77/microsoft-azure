...
class MultiTurnBot {
        ...
        this.dialogs.add(new NumberPrompt(AGE_PROMPT, async (prompt) => {
            if (prompt.recognized.succeeded) {
                if (prompt.recognized.value <= 0) {
                    await prompt.context.sendActivity(`Dein Alter kann nicht negativ sein.`);
                    return false;
                } else {
                    return true;
                }
    ...
    async promptForName(step) {
        return await step.prompt(NAME_PROMPT, `Wie heißt du?`);
    }
    ...
    async confirmAgePrompt(step) {
        ...
        await step.prompt(CONFIRM_PROMPT, 'Willst du mir dein Alter sagen?', ['Ja', 'Nein']);
    }
    ...
    async promptForAge(step) {
        if (step.result && step.result.value === 'Ja') {
            return await step.prompt(AGE_PROMPT, `Wie alt bist du?`,
                {
                    retryPrompt: 'Entschuldigung, bitte gib dein Alter als Zahl an oder sag cancel.'
                }
            );
        } else {
    ...
    async captureAge(step) {
        ...
        if (step.result !== -1) {
            ...
            await this.userProfile.set(step.context, user);
            await step.context.sendActivity(`Ich weiß jetzt, dass du ${ step.result } Jahre als bist.`);
        } else {
            await step.context.sendActivity(`Kein Alter angegeben.`);
        }
    ...
    async displayProfile(step) {
        const user = await this.userProfile.get(step.context, {});
        if (user.age) {
            await step.context.sendActivity(`Du heißt ${ user.name } und du bist ${ user.age } Jahre alt.`);
        } else {
            await step.context.sendActivity(`Du heißt ${ user.name } und du hast mir dein Alter nicht verraten.`);
        }
        return await step.endDialog();
    }
