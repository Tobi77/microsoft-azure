...
class SuggestedActionsBot {
    ...
    async onTurn(turnContext) {
        ...
        if (turnContext.activity.type === ActivityTypes.Message) {
            ...
            const validColors = ['Rot', 'Blau', 'Gelb', 'Grün'];

            // If the `text` is in the Array, a valid color was selected and send agreement.
            if (validColors.includes(text)) {
                await turnContext.sendActivity(`Da stimme ich dir zu, ${ text } ist die Beste Farbe.`);
            } else {
                await turnContext.sendActivity('Bitte wähle eine Farbe.');
            }
    ...
    async sendSuggestedActions(turnContext) {
        var reply = MessageFactory.suggestedActions(['Rot', 'Gelb', 'Blau', 'Grün'], 'Was ist die beste Farbe?');
