    ...
    async onTurn(turnContext) {
        if (turnContext.activity.type === ActivityTypes.Message) {
            ...
            if (!turnContext.responded && results.status === DialogTurnStatus.empty) {
                await turnContext.sendActivity('Willkommen zum Rich Cards Bot!');
                // Create the PromptOptions which contain the prompt and re-prompt messages.
                // PromptOptions also contains the list of choices available to the user.
                const promptOptions = {
                    prompt: 'Bitte wähle eine Karte:',
                    retryPrompt: 'Das war keine gültige Eingabe, bitte wähle eine Karte oder eine Zahl zwischen 1 und 8.',
        ...
        const cardOptions = [
            {
                ...
                synonyms: ['1', 'eins', 'animation', 'animation card']
            },
            {
                value: 'Audio Card',
                synonyms: ['2', 'zwei', 'audio', 'audio card']
            },
            {
                value: 'Hero Card',
                synonyms: ['3', 'drei', 'hero', 'hero card']
            },
            {
                value: 'Receipt Card',
                synonyms: ['4', 'vier', 'receipt', 'receipt card']
            },
            {
                value: 'Signin Card',
                synonyms: ['5', 'fünf', 'signin', 'signin card']
            },
            {
                value: 'Thumbnail Card',
                synonyms: ['6', 'sechs', 'thumbnail', 'thumbnail card']
            },
            {
                value: 'Video Card',
                synonyms: ['7', 'sieben', 'video', 'video card']
            },
            {
                value: 'All Cards',
                synonyms: ['8', 'acht', 'all', 'all cards']
            }
        ];
