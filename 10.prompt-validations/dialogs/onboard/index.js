...
class OnboardingDialog extends ComponentDialog {
    ...
    async promptForName(step) {
        ...
        if (user.name) {
            ...
            return await step.prompt(GET_NAME_PROMPT, `Wie heißt du, Mensch?`);
        }
    }
    ...
    async promptForAge(step) {
        ...
        return await step.prompt(GET_AGE_PROMPT, `Wie alt bist du?`);
    }

    // Collect age, then prompt for date of birth.
    async promptForDob(step) {
        step.values[AGE_PROPERTY] = step.result;
        return await step.prompt(GET_DOB_PROMPT, `Wann ist dein Geburtstag?`);
    }
    ...
    async promptForColor(step) {
        ...
        const choices = ['Rot', 'Blau', 'Grün'];
        return await step.prompt(GET_COLOR_PROMPT, `Zum Schluss, was ist deine Lieblingsfarbe?`, choices);
    }
    ...
    async completeProfile(step) {
        ...
        await step.context.sendActivity(`Dein Profil ist komplett! Vielen Dank.`);
    ...
    async displayProfile(step) {
        ...
        const text = [
            `Ich nenne dich "${ user[USER_NAME_PROPERTY] }".`,
            `Du bist geboren am ${ moment(user[DOB_PROPERTY]).format('MMM Do, YYYY') } und du behauptest, du bist ${ user[AGE_PROPERTY] } Jahre alt.`,
            `Deine Lieblingsfarbe ist ${ user[COLOR_PROPERTY] }.`
        ];
